    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6">
            <div class="mt-2 flex items-center text-sm text-gray-500">
                ...
                @if($order->lastStatus ) {{ __('Status: ') . $order->lastStatus->label }} @endif
            </div>
            <div class="mt-2 flex items-center text-sm text-gray-500">
            ...
            <div class="mt-2 flex items-center text-sm text-gray-500">
                ...
                @if(!is_null($order->created_at)) {{ __('Created ') . $order->created_at->diffForHumans() }} @endif
            </div>
        </div>
    </div>
    <div class="mt-5 flex lg:mt-0 lg:ml-4">
        <livewire:client.order.appointment-form :order="$order" />
        
        

        @if ($order->lastStatus && strtolower($order->lastStatus->label) == 'pending')
            
            <span class="sm:ml-3">
               
                <a href="{{ route('orders.process', $order->reference) }}" 
