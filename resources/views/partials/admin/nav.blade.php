...
<x-common.nav-dropdown :active="request()->routeIs('admin.orders.*')">
    ...
    <x-common.nav-link :href="route('admin.orders.status',['status'=> 'pickedup'])" :active="request()->routeIs('admin.orders.status') && request()->route('status')== 'pickedup'">
    {{__('Reception')}}
    </x-common.nav-link>

    <x-common.nav-link  :href="route('admin.orders.status',['status'=> 'readytopickup'])" :active="request()->routeIs('admin.orders.status') && request()->route('status')== 'readytopickup'" >  
    {{__("Pret pour l'enlevement")}}
    </x-common.nav-link>

    <x-common.nav-link :href="route('admin.orders.status',['status'=> 'packagingprocessing'])" :active="request()->routeIs('admin.orders.status') && request()->route('status')== 'packagingprocessing'">
    {{__('Encours de conditionnement')}}
    </x-common.nav-link>

    <x-common.nav-link :href="route('admin.orders.status',['status'=> 'paid'])" :active="request()->routeIs('admin.orders.status') && request()->route('status')== 'paid'">
    {{__('Payé')}}
    </x-common.nav-link>

    <x-common.nav-link :href="route('admin.orders.status',['status'=> 'pending'])" :active="request()->routeIs('admin.orders.status')  && request()->route('status')== 'pending'">
    {{__('Pending')}}
    </x-common.nav-link>
...
<x-common.nav-dropdown :active="request()->routeIs('admin.docks.*')">
    ...
    <x-common.nav-link :href="route('admin.docks.packagings.status',['status'=> 'pending'])" :active="request()->routeIs('admin.docks.packagings.status')  && request()->route('status')== 'pending'">
    {{__('attente')}}
    </x-common.nav-link>
...
<x-common.nav-dropdown :active="request()->routeIs('admin.appointments.*')">
    <x-slot name="title">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <x-common.nav-link :href="route('admin.appointments.index')" :active="request()->routeIs('admin.appointments.index')">
    {{__('tout')}}
    </x-common.nav-link>

    <x-common.nav-link :href="route('admin.appointments.status',['status'=> 'assigned'])" :active="request()->routeIs('admin.appointments.status')  && request()->route('status')== 'assigned'">
    {{__('assigné')}}
    </x-common.nav-link>

    <x-common.nav-link :href="route('admin.appointments.status',['status'=> 'pending'])" :active="request()->routeIs('admin.appointments.status')  && request()->route('status')== 'pending'">
    {{__('attente')}}
    </x-common.nav-link>

    <x-common.nav-link :href="route('admin.appointments.status',['status'=> 'honored'])" :active="request()->routeIs('admin.appointments.status')  && request()->route('status')== 'honored'">
    {{__('honnoré')}}
    </x-common.nav-link>

    <x-common.nav-link :href="route('admin.appointments.status',['status'=> 'canceled'])" :active="request()->routeIs('admin.appointments.status')  && request()->route('status')== 'canceled'">
    {{__('annulé')}}
    </x-common.nav-link>
