...
<x-modal.dialog wire:model.defer="showAssignModal" cssWidth="sm:w-6/12">
    <x-slot name="title">{{__("Assign to transporters")}}</x-slot>

    <x-slot name="content">
        <div class="font-extrabold tracking-tight leading-none sm:border-t sm:py-5">
            {{__('Select  transporters')}}
        </div>
                            ...
                            @forelse ($transporters as $transporter)
                                <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $transporter->id }}">
                                    <x-table.cell class="pr-0">
                                        <x-input.checkbox wire:model="transporter" value="{{ $transporter->id }}" />
                                    </x-table.cell>
                                    <x-table.cell>
    ...
    <x-slot name="footer">
        <x-button.secondary wire:click="$set('showAssignModal', false)">Annuler</x-button.primary>
